#!/usr/bin/env node
/**
 * One-off recovery script: Batch 4 (FINAL) historical missing invoices
...
const BATCH_IDS = ['211534084','211552577','211691387','211802611','211911140','211971492','211984973','212082895'];
const BATCH_NAME = 'BATCH4';
...
async function main() {
  log('Starting ' + BATCH_NAME + ' (FINAL) historical recovery for ' + BATCH_IDS.length + ' order IDs');
  logTakealotEvent({ source: 'batch4-recovery', event_type: 'batch4_recovery_started', level: 'info',
    message: BATCH_NAME + ' recovery started', context: { order_ids: BATCH_IDS } });

  const lineItems = JSON.parse(fs.readFileSync('/home/stan/reports/batch4-line-items.json', 'utf8'));
  const grouped = {};
      ...
      if (createResp.success) {
        for (const r of (createResp.results || [])) {
          if (r.status === 'invoiced') {
            ...
            logTakealotEvent({ source: 'batch4-recovery', event_type: 'order_invoice_created', level: 'info',
  ...
  log(BATCH_NAME + ' (FINAL) recovery done. Created: ' + created + ', Errors: ' + errors);
  logTakealotEvent({ source: 'batch4-recovery', event_type: 'batch4_recovery_completed', level: 'info',
...
main().catch(e => {
  log('FATAL: ' + e.message);
  logTakealotEvent({ source: 'batch4-recovery', event_type: 'batch4_recovery_failed', level: 'error',
